...
public class PackageChecker {
    ...
    public static void showStatusBarError(String message) {
        if (project == null) {
            logger.error("Project must be initialized before showing status bar errors");
            return;
        }
        ...
        logger.info("Status bar error shown: " + message);
    }
    ...
    public static void checkPackagesDefinedInXml(Set<String> usedPackages, Set<String> xmlPackages) {
        ...
        for (String usedPackage : usedPackages) {
            if (!xmlPackages.contains(usedPackage)) {
