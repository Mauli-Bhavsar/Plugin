...
public class AnnotationChecker {
    ...
    ConsoleProgressIndicator indicator = new ConsoleProgressIndicator(6);
    private final Set<AnnotationStage> completedStages = EnumSet.noneOf(AnnotationStage.class);

    /**
     * Stages of annotation checking.
     */
    private enum AnnotationStage {
        AUTOWIRED_FIELDS_AND_PARAMETERS,
        MISSING_SERVICE_ANNOTATION,
    ...
    private void updateProgress(AnnotationStage stage, String message, double fraction) {
    ...
    public void checkAnnotations(@NotNull PsiClass psiClass, @NotNull Project project) {
        updateProgress(AnnotationStage.AUTOWIRED_FIELDS_AND_PARAMETERS, "Adding already autowired annotated fields and parameters...", 0.16);
        addAutowiredFields(psiClass);
        addConstructorParameters(psiClass);
        addSetterMethodParameters(psiClass);

        updateProgress(AnnotationStage.MISSING_SERVICE_ANNOTATION, "Checking missing service and having autowired annotation...", 0.32);
        if (!hasServiceAnnotation(psiClass) && hasAutowiredFields(psiClass)) {
            warnMissingServiceAnnotation(psiClass, project);
        }

        updateProgress(AnnotationStage.AUTOWIRED_ANNOTATION, "Checking for autowired annotation having service annotation...", 0.48);
        if (hasServiceAnnotation(psiClass)) {
            ...
            updateProgress(AnnotationStage.DUPLICATE_BEANS, "Checking for duplicate beans...", 0.64);
            ...
            updateProgress(AnnotationStage.CYCLIC_DEPENDENCIES, "Checking for cyclic dependencies...", 0.80);
            ...
            updateProgress(AnnotationStage.PAYLOAD, "Checking for payload...", 0.96);
