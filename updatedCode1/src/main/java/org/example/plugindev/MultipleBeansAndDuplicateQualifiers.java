...
public class MultipleBeansAndDuplicateQualifiers {
    ...
    private void handleQualifierAnnotation(PsiAnnotation qualifierAnnotation, String elementDescription, List<PsiClass> allChildClasses, Project project) {
        ...
        else if (!isValidQualifierName(qualifierName, allChildClasses)) {
     ...
     */
    private boolean isValidQualifierName(String qualifierName, List<PsiClass> allChildClasses) {
        for (PsiClass childClass : allChildClasses) {
            if (qualifierName.equals(decapitalize(childClass.getName())) || hasMatchingClassQualifierAnnotation(qualifierName, childClass)) {
                return true;
            }
        }
        return false;
