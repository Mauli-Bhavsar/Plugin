...
public class MultipleBeansAndDuplicateQualifiers {
    ...
    public static void showStatusBarError(Project project, String message) {
    ...
    private void showEditorNotification(List<PsiClass> psiClasses, String message, Project project) {
        EditorNotifications.updateAll();
        for (PsiClass psiClass : psiClasses) {
            new EditorNotifications.Provider<>() {
                @Override
                public @NotNull Key<JComponent> getKey() {
                    return Key.create("annotation.notification");
                }

                @Override
                public EditorNotificationPanel createNotificationPanel(@NotNull VirtualFile file, @NotNull FileEditor fileEditor, @NotNull Project project) {
                    if (!psiClass.getContainingFile().getVirtualFile().equals(file)) {
                        return null;
                    }
            ...
            if (serviceAnnotatedClasses.size() > 1) {
                ...
                if (primaryAnnotatedClasses.size() > 1) {
                    ...
                    if(field!=null)
                    {
                        ...
                        showStatusBarError(project, message);
                        showEditorNotification(allChildClasses, message, project);
                    }
                    if(parameter!=null)
                    {
...
//        List<PsiClass> allChildClasses = new ArrayList<>();
...
//            allChildClasses.add(childClass);
//        }
...
//    }
    private List<PsiClass> findAllChildClasses(@NotNull PsiClass parentClass, @NotNull Project project) {
        ...
        GlobalSearchScope projectScope = GlobalSearchScope.allScope(project);
        findAllChildClassesRecursive(parentClass, project, projectScope, allChildClassesSet);
        return new ArrayList<>(allChildClassesSet);
    }

    private void findAllChildClassesRecursive(PsiClass parentClass, Project project, GlobalSearchScope projectScope, Set<PsiClass> allChildClassesSet) {
        Query<PsiClass> query = ClassInheritorsSearch.search(parentClass, projectScope, true);
        for (PsiClass childClass : query) {
            if (!allChildClassesSet.contains(childClass)) {
                ...
                if (childClass.isInterface() || childClass.hasModifierProperty(PsiModifier.ABSTRACT)) {
                    findAllChildClassesRecursive(childClass, project, projectScope, allChildClassesSet);
                }
            }
        }
    }
...
//            showEditorNotification(allChildClasses, message, project);
    ...
    private void handleQualifierAnnotation(PsiAnnotation qualifierAnnotation, String elementDescription, List<PsiClass> allChildClasses, Project project) {
        ...
        if (qualifierName == null || qualifierName.trim().isEmpty()) {
            ...
            showStatusBarError(project, errorMessage);
            showEditorNotification(allChildClasses, errorMessage, project);
        }
    ...
    private boolean isValidQualifierName(String qualifierName, List<PsiClass> allChildClasses,Project project) {
        for (PsiClass childClass : allChildClasses) {
            if (qualifierName.equals(decapitalize(childClass.getName())) || hasMatchingClassQualifierAnnotation(qualifierName, childClass,project)) {
                return true;
            }
        }
        return false;
     ...
     */
    private boolean hasMatchingClassQualifierAnnotation(String qualifierName, PsiClass childClass,Project project) {
