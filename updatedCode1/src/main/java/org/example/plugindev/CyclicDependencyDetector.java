...
public class CyclicDependencyDetector {
    ...
    public static void showStatusBarErrorForCycleDetection(Project project, String message) {
    ...
    private void showEditorNotificationForCycleDetection(String cyclePath, Project project) {
        EditorNotifications.updateAll();
        new EditorNotifications.Provider<>() {
            @Override
            public @NotNull @org.jetbrains.annotations.NotNull Key<JComponent> getKey() {
                return Key.create("annotation.notification");
            }

            @Override
            public EditorNotificationPanel createNotificationPanel(@NotNull @org.jetbrains.annotations.NotNull VirtualFile file, @NotNull @org.jetbrains.annotations.NotNull FileEditor fileEditor, @NotNull @org.jetbrains.annotations.NotNull Project project) {
                EditorNotificationPanel panel = new EditorNotificationPanel();
                panel.setText(cyclePath);
                return panel;
            }
        };
        EditorNotifications.getInstance(project).hashCode();
    }
    ...
    private void handleCycleDetection(List<PsiClass> currentPath, PsiClass psiClass, Set<String> visitedCycles, Project project) {
        ...
        if (!visitedCycles.contains(cyclePath)) {
            ...
            if (currentPath.size() == 1) {
                ...
                showStatusBarErrorForCycleDetection(project, errorMessage);
                showEditorNotificationForCycleDetection(cyclePath, project);
            } else {
        ...
        if (fieldType instanceof PsiClassType) {
            ...
            if (fieldClass != null) {
                ...
                if(!field.hasAnnotation(QUALIFIER_ANNOTATION)) {
                    ...
                    if(childClasses.size()==2 && childClasses.contains(psiClass))
                    {
